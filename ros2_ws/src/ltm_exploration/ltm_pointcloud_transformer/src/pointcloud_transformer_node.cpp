...
void PointCloudTransformerNode::pointcloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  transformPointCloud(msg, cloud_output, m_target_frame);
  ...
  sensor_msgs::msg::PointCloud2::SharedPtr cloud_output, const std::string &target_frame)
{
  ...
  try {
    geometry_msgs::msg::TransformStamped transform_stamped = m_tf_buffer->lookupTransform(
      target_frame, cloud_input->header.frame_id, cloud_input->header.stamp);
    pcl_ros::transformPointCloud(target_frame, transform_stamped, *cloud_input, *cloud_output);
...
void PointCloudTransformerNode::initializeParameters()
{
  ...
  declare_parameter("target_frame", "base_footprint");
